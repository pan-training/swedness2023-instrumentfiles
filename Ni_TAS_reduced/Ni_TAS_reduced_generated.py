...
# Instrument: Ni_TAS_reduced.instr (Ni_TAS_reduced)
# Date:       Wed Sep  6 07:19:03 2023
# File:       Ni_TAS_reduced_generated.py

import mcstasscript as ms

# Python McStas instrument description
instr = ms.McStas_instr("Ni_TAS_reduced_generated", author = "McCode Py-Generator", origin = "ESS DMSC")

# Add collected DEPENDENCY strings
instr.set_dependency(' -I@MCCODE_LIB@/share/ -DFUNNEL ')

# *****************************************************************************
# * Start of instrument 'Ni_TAS_reduced' generated code
# *****************************************************************************
# MCSTAS system dir is "/Users/pkwi/McStas/mcstas/3.x-dev/"


# *****************************************************************************
# * instrument 'Ni_TAS_reduced' and components DECLARE
# *****************************************************************************

# Instrument parameters:

EI = instr.add_parameter('double', 'EI', value=5, comment='Parameter type (double) added by McCode py-generator')
EF = instr.add_parameter('double', 'EF', value=5, comment='Parameter type (double) added by McCode py-generator')
QHK = instr.add_parameter('double', 'QHK', value=0, comment='Parameter type (double) added by McCode py-generator')
...
if (!anglemode) {
  if (!EI && !EF)
      exit(fprintf(stderr,
        "Ni_TAS_reduced: ERROR: neutron beam energy is not defined (EI, EF, KI, KF)\n"));
  ...
  if (!QM && !QH && !QK && !QL)
    exit(fprintf(stderr,
      "Ni_TAS_reduced: ERROR: No Q transfer defined (QM, QH, QK, QL)\n"));
  ...
  if (!QM && qs>=0) QM = sqrt(qs);
      ...
      "Ni_TAS_reduced: Warning: Can not compute Q-modulus from A4=%g [deg].\n",A4);
}
...
if (verbose) {
  printf("Ni_TAS_reduced: Detailed TAS configuration\n");
  printf("* Incoming beam: EI=%.4g [meV] KI=%.4g [Angs-1] Vi=%g [m/s]\n", EI, KI, Vi);
  printf("* Outgoing beam: EF=%.4g [meV] KF=%.4g [Angs-1] Vf=%g [m/s]\n", EF, KF, Vf);
}
...
if (machine_hkl.l1 <= 1)
  fprintf(stderr, "Ni_TAS_reduced: Warning: L1 too short. Min=1\n");

if (machine_hkl.l2 <= 0.35)
  exit(fprintf(stderr, "Ni_TAS_reduced: ERROR: L2 too short. Min=0.35\n"));

if (machine_hkl.l3 <= 0.40)
  exit(fprintf(stderr, "Ni_TAS_reduced: ERROR: L3 too short. Min=0.40\n"));

if (machine_hkl.l4 <= 0.24)
  exit(fprintf(stderr, "Ni_TAS_reduced: ERROR: L4 too short. Min=0.24\n"));

if (!anglemode) {
  machine_real = qhkl2angles(sample, machine_hkl, machine_real);
  if (strlen(machine_real.message))
    exit(fprintf(stderr, "Ni_TAS_reduced: ERROR: %s [qhkl2angles]\n", machine_real.message));
}
...
# *****************************************************************************
# * instrument 'Ni_TAS_reduced' TRACE
# *****************************************************************************
...
# end of generated Python code Ni_TAS_reduced_generated.py 
